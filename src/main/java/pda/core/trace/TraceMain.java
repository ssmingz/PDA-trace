...
public class TraceMain {
    ...
    private static void special_config(Subject subject) {
        String buildConfigFileName = "mockito-core.bnd";
        String base = Utils.join(subject.getHome(), "conf");
        String newConfigFile = Utils.join(Constant.D4J_MOCKITO_CONF, String.valueOf(subject.getId()));
        File original = new File(Utils.join(base, buildConfigFileName));
        File backup = new File(Utils.join(base, buildConfigFileName + ".bak"));
        // backup the original config file
        if (original.exists() && !backup.exists()) {
            Utils.moveFile(original.getAbsolutePath(), backup.getAbsolutePath());
        }
        // replace the configure file
        Utils.copyFile(Utils.join(newConfigFile, buildConfigFileName), Utils.join(base, buildConfigFileName));
    }
    ...
    public static void main(String[] args) {
        ...
        for (Subject subject : subjects) {
            try {
                // mockito project cannot be compiled when referring outside classes
                // the compiling script should be replaced
                if ("mockito".equals(subject.getName()))  {
                    special_config(subject);
                }
